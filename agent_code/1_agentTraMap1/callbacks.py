...
# State definition
    ...
    if (len(coins) > 0):
        ...
        dist = np.sum(np.abs(np.subtract(coins, curr)), axis=1)

        idx_coin = np.argmin(dist)

        if (curr[1] - coins[idx_coin][1]  > 0):
            state[4] = 1

        if (curr[1] - coins[idx_coin][1]  < 0):
            state[5] = 1

        if (curr[0] - coins[idx_coin][0]  > 0):
            state[6] = 1
...
def end_of_episode(self):
    ...
    print(self.nepisodios,TempValScore,TempValSteps,TempValEp )
    ...
    #for (state, qtemp) in self.StatesIndex:
    #    print(state, qtemp, np.argmax(qtemp), self.ActMap[np.argmax(qtemp)])
